...
import { teams, byCode, matches, odds, score, h2h, factors } from '../data/wc.js';
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {list.map((pl) => (
          <div key={pl.team + pl.idx} className="card press" onClick={() => onOpenPlayer(pl.team, pl.idx)} style={{ padding: 12, display: 'flex', alignItems: 'center', gap: 11 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 900, fontSize: 15, color: '#794f27' }}>{pl.name}</div>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#9f927d' }}>{byCode[pl.team].name} · {pl.pos} · {pl.age}岁</div>
            </div>
            <OvrBadge value={pl.ovr} />
          </div>
        ))}
...
/* ---------------- 球员详情 (五边形) ---------------- */
export function PlayerDetailScreen({ code, idx }) {
  const t = byCode[code];
