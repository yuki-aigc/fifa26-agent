/* ===========================================================
   世界杯2026 预测系统 · 数据 + 胜率引擎
   导出 { teams, byCode, matches, odds, score, h2h, factors }
   =========================================================== */
...
export const byCode = Object.fromEntries(teams.map((t) => [t.code, t]));
...
export function odds(aCode, bCode) {
...
// 预测比分
export function score(aCode, bCode) {
  const A = byCode[aCode], B = byCode[bCode];
...
// 历史交锋 (由队代码确定性生成)
export function h2h(aCode, bCode) {
...
// 关键因素 (用于详情页解读)
export function factors(aCode, bCode) {
  const A = byCode[aCode], B = byCode[bCode];
  const f = [];
